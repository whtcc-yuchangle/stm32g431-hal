...
{
    uint8_t wbuf[10] = {"whtcc"};
    uint8_t rbuf[10] = {"\0"};
    
    char buf[20] = {'\0'};

    at24c02_i2c_write(wbuf, 6, 0x00);
    at24c02_i2c_read(rbuf, 6, 0x00);
    
    sprintf(buf, "AT24C02:%s   ", rbuf);
...
void mcp40xx_i2c_test(void)
{
    ...
    mcp40xx_i2c_write(wbuf);
    
    mcp40xx_i2c_read(&rbuf);
    
    if (wbuf == rbuf)
    {
        sprintf(result, "OK. [%d, %d]", wbuf, rbuf);
    }
    else
    {
        sprintf(result, "FAIL. [%d, %d]", wbuf, rbuf);
    }
    
    LCD_DisplayStringLine(Line4, (uint8_t *)result);
}
